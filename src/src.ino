...
PanelState currentState = TESTING;
...
void loop() {
    ...
    if (currentState == SET_MOVING)
    {
      ...
      for (uint8_t i = 0; i < coarseSensor.num_sensors; i++)
      {
        ...
        {
          coarseSensor.sensors[i].intensity = (float)(sensorIntensity[i][totalNumReadings/2] + sensorIntensity[i][totalNumReadings/2  - 1]) / 2;
        }
        else
        {
...
/// @brief Sorts the sensor readings (between different sensors) in descending order of intensity but with the elevation sensor at the end
...
//    angle_estimate.elevation = lerp(coarseSensor.sensors[4].elPosition, coarseSensor.sensors[0].elPosition, elWeight);
    float elWeight = 1.0 - (((float)coarseSensor.sensors[4].intensity) / (coarseSensor.sensors[4].intensity + coarseSensor.sensors[0].intensity));
